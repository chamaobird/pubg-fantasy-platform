...
- **start_date / end_date — armadilha de timezone**: campos são `TIMESTAMPTZ`; admin form envia apenas a parte de data (ex: "2026-05-01"); o backend armazena como midnight UTC (`2026-05-01T00:00:00+00:00`); no frontend, `new Date("2026-05-01T00:00:00Z")` em EDT (UTC-4) = "30 abr 20:00". Fix: usar `parseDateLocal(iso)` = `new Date(y, m-1, d)` (midnight local) ao exibir `start_date`/`end_date` como datas de calendário — implementado no Dashboard em `buildDateLabel`/`buildDateRange`
