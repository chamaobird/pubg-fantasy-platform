...
class Lineup(Base):
    ...
    user_id: Mapped[str] = mapped_column(
        String(60), nullable=False, index=True, comment="Auth user identifier (UUID string)"
    )
    ...
    total_cost: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True, comment="Sum of locked costs at lineup_close_at"
    )
    total_points: Mapped[Optional[Decimal]] = mapped_column(
        Numeric(10, 2), nullable=True, comment="Filled after day scoring"
    )
    ...
    players: Mapped[List["LineupPlayer"]] = relationship(
        "LineupPlayer", back_populates="lineup", cascade="all, delete-orphan", lazy="select"
    )
...
class LineupPlayer(Base):
    ...
    lineup_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("lineup.id", ondelete="CASCADE"), nullable=False, index=True
    )
    ...
    points_earned: Mapped[Optional[Decimal]] = mapped_column(
        Numeric(8, 2), nullable=True, comment="Points credited to user from this player"
    )
    ...
    def __repr__(self) -> str:
        return (
            ...
            f"roster_id={self.roster_id} slot={self.slot_type!r}>"
        )
